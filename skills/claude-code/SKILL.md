...
4. `status("Editing: <file>", batchId)` per batch, working through batches in order (use the selector, class name, `react.source` to pin the component); each batch carries its own `page`. The browser follows the batch you are working on (its page), so calling `status(…, batchId)` before editing matters.
